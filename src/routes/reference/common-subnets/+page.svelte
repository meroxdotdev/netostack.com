...
<div class="card">
	...
	<div class="subnets-table fade-in">
		...
		{#each COMMON_SUBNETS as subnet}
			<Tooltip text={`/${subnet.cidr} with ${subnet.mask} supports ${subnet.hosts.toLocaleString()} hosts`} position="top">
				<div class="table-row">
...
<style lang="scss">
	.subnets-table {
		...
		:global(.tooltip-container) { width: 100%; }
	}
	...
	.table-header {
		...
		> span {
			...
			&:nth-child(1), &:nth-child(2), &:nth-child(4) { justify-content: flex-start; }
	...
	.cidr-cell {
		font-weight: 700; color: var(--color-info-light); justify-content: flex-start;
		@media (max-width: 768px) { &::before { content: 'CIDR: '; } }
	}

	.mask-cell {
		font-size: var(--font-size-sm); color: var(--text-primary); justify-content: flex-start;
		@media (max-width: 768px) { &::before { content: 'Mask: '; } }
	}

	.hosts-cell {
		font-size: var(--font-size-sm); color: var(--color-success-light); justify-content: flex-end;
		@media (max-width: 768px) { &::before { content: 'Hosts: '; } }
	}

	.usage-cell {
		font-size: var(--font-size-sm); color: var(--text-secondary); justify-content: flex-start;
		font-family: inherit;
		@media (max-width: 768px) { &::before { content: 'Usage: '; } }
	}
</style>
