...
<div class="card">
	...
	<div class="reference-section fade-in">
		{#each Object.entries(NETWORK_CLASSES) as [className, classInfo]}
			<Tooltip text={`Class ${className} networks use ${classInfo.defaultMask} (/${classInfo.cidr}) and cover ${classInfo.range}`} position="top">
...
<style lang="scss">
	...
	.class-info { display: flex; align-items: center; gap: var(--spacing-sm); }
	...
	.class-details {
		h3 { margin: 0; color: var(--text-primary); font-weight: 600; }
	}
	...
	.class-description { font-size: var(--font-size-sm); color: var(--text-primary); margin-bottom: var(--spacing-sm); }
	.usage-info { font-size: var(--font-size-xs); color: var(--text-secondary); }
</style>
