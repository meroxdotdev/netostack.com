...
async function checkTCPPort(host: string, port: number, timeout: number = 5000): Promise<{ open: boolean, latency?: number }> {
...
async function checkMXHealth(domain: string, checkPorts: boolean = false): Promise<any> {
  ...
  const healthyMX = mxRecords.filter(mx => !mx.error && mx.addresses && 
    (mx.addresses.ipv4.length > 0 || mx.addresses.ipv6.length > 0)).length;
  const reachableMX = checkPorts ? 
    mxRecords.filter(mx => mx.portChecks?.some(p => p.open)).length : null;
...
async function checkSPF(domain: string): Promise<any> {
  ...
  return {
    ...
    emailAnalysis: {
      ...
      deliverabilityRisk: result.record?.includes('-all') ? 'low' : 
                         result.record?.includes('~all') ? 'medium' : 'high'
    }
  };
}
