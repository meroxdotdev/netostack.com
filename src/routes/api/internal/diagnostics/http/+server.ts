...
function analyzeSecurityHeaders(headers: Headers): {
  ...
  const analysis: Array<{ header: string; status: 'present' | 'missing' | 'weak'; message: string; recommendation?: string }> = [];
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    const { action, url, method = 'GET', headers: customHeaders = {}, maxRedirects = 10, timeout = 10000 } = await request.json();
    ...
    switch (action) {
      ...
      case 'redirect-trace': {
        const { response, timings, redirectChain } = await performHTTPRequest(url, method, customHeaders, true, maxRedirects, timeout);
