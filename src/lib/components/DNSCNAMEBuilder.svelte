<script lang="ts">
  ...
  let results = $state<{ alias: string; target: string; ttl: number; status: 'valid' | 'loop' | 'self-target' | 'invalid-format' | 'missing-dot' }[]>([]);
  ...
  function validateCNAME(alias: string, target: string, allRecords: typeof results): 'valid' | 'loop' | 'self-target' | 'invalid-format' | 'missing-dot' {
...
<div class="card">
  <div class="card-header">
    ...
    <p class="card-subtitle">
      Build valid CNAME records with loop detection, self-target checks, and FQDN validation.
    </p>
  </div>
    ...
    <div class="results-section">
      <div class="results-header">
        ...
        <div class="export-buttons">
          <button onclick={() => copyToClipboard(results.map(r => `${r.alias} ${r.ttl} IN CNAME ${r.target}`).join('\n'))}>
