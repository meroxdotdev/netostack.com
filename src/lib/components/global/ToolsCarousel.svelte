<script lang="ts">
  ...
  const durationFor = (count: number) =>
    Math.max(12, Math.round(speedBase * (count / 8))); // scale with item count
...
<div class="tools-carousel" style="--gap:{gap}; --card-width:{cardWidthCss};">
  {#each rows as row, i}
    ...
    >
      ...
      <div
        class="track"
        style="animation-duration: {durationFor(row.items.length)}s;"
      >
...
.track {
  ...
  /* direction per row */
  .row[data-dir="ltr"] & { animation-name: scroll-right; } /* visual motion L→R */
  .row[data-dir="rtl"] & { animation-name: scroll-left;  } /* visual motion R→L */
}
...
@media (max-width: 768px) {
  .tools-carousel { gap: calc(var(--gap) * 0.75); }
}
</style>
