<script>
	...
	const flagOptions = [
		...
		{ value: 'S', label: 'S - Terminal rule (SRV)', description: 'The Rule is terminal and the result is for SRV lookup' },
		{ value: 'A', label: 'A - Terminal rule (Address)', description: 'The Rule is terminal and the result is an address record' },
	...
	let isValid = $derived.by(() => {
		return domain.trim() !== '' && 
			   order !== '' && 
			   preference !== '' &&
			   parseInt(order) >= 0 && parseInt(order) <= 65535 &&
			   parseInt(preference) >= 0 && parseInt(preference) <= 65535;
	});
	...
	function copyToClipboard() {
		navigator.clipboard.writeText(naptrRecord);
			...
			<div class="main-grid">
				...
				<div class="input-section">
					...
					<div class="order-grid">
						<div class="input-group">
							<label for="order" use:tooltip={"Processing order - lower values are processed first (0-65535)"}>Order *</label>
							<input
								id="order"
								type="number"
								bind:value={order}
						...
						<div class="input-group">
							<label for="preference" use:tooltip={"Preference within the same order value (0-65535)"}>Preference *</label>
							<input
								id="preference"
								type="number"
					...
					<div class="input-group">
						<label for="flags" use:tooltip={"Control processing behavior - affects how the result is interpreted"}>Flags</label>
					...
					<div class="input-group">
						<label for="service" use:tooltip={"Service identifier or protocol (e.g., E2U+sip for SIP services)"}>Service</label>
						<input
							id="service"
							type="text"
							bind:value={service}
					...
					<div class="input-group">
						<label for="regexp" use:tooltip={"Regular expression for pattern matching and substitution (format: !pattern!replacement!)"}>Regular Expression</label>
						<input
							id="regexp"
							type="text"
							bind:value={regexp}
