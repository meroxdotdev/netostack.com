...
export function planSubnets(
  ...
  try {
    ...
    const sortedRequests = strategy === 'fit-best' 
    ...
    for (const request of sortedRequests) {
      ...
      const subnet = createAllocatedSubnet(
        request,
        alignedAddress,
        request.prefix!,
        parent.version,
    ...
    const leftover = findLeftoverSpace(
      parent.network,
      parent.broadcast,
      allocatedRanges,
      parent.version
    ...
    const totalAllocatedAddresses = allocatedRanges.reduce(
      (sum, range) => sum + (range.end - range.start + 1n),
      0n
    );
    const totalLeftoverAddresses = leftover.reduce(
      (sum, block) => sum + BigInt(block.size.replace(/,/g, '')),
      0n
    );
