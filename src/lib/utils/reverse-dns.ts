  ...
  try {
    ...
    if (isValidIPv6(network)) {
      ...
      if (prefixLength >= 24) {
        ...
        for (let i = 0; i < Math.min(numSubnets, 10); i++) { // Limit output
...
export function analyzePTRCoverage(
  cidr: string,
  existingPTRs: string[],
  ...
  const missingPTRs = expectedPTRs
    .filter(r => !existingPTRSet.has(r.ptrName))
    .map(r => r.ptrName);
  ...
  const coverage = expectedPTRs.length > 0 
    ? ((expectedPTRs.length - missingPTRs.length) / expectedPTRs.length) * 100 
    : 0;
