...
function checkContainment(
  ...
  if (coverage === 0) {
    ...
    if (strictEquality && mergedCovered.length === 1 && 
...
export function computeCIDRContains(
  ...
  const linesA = inputA.trim().split('\n').filter(line => line.trim());
  const linesB = inputB.trim().split('\n').filter(line => line.trim());
  ...
  // Counters
  let inside = 0, equal = 0, partial = 0, outside = 0;
