...
export function expandIPv6(address: string): string {
  ...
  return cleanAddress.split(':').map(part => part.padStart(4, '0')).join(':');
}
...
function calculateIPv6AddressRange(networkAddress: string, prefixLength: number): {
  first: string;
  last: string;
} {
...
export function calculateIPv6Subnet(
  ...
  try {
    ...
    const assignableAddresses = prefixLength >= 127 ? '0' : calculateIPv6TotalAddresses(prefixLength).replace(/[,\s]/g, '') === '2' ? '0' : 
      prefixLength <= 53 ? (Math.pow(2, 128 - prefixLength) - 2).toLocaleString() : 'Nearly all addresses';
