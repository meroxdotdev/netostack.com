...
const SUPPORTED_FLAGS = (() => {
  let out = "";
  for (const f of CANDIDATE_FLAGS) try { new RegExp("", f); out += f; } catch {}
  return out;
})();
const supportsLookbehind = (() => { try { new RegExp("(?<=x)y"); return true; } catch { return false; } })();

const splitRegexLiteral = (src: string): { pattern: string; flags: string } | null => {
  if (!src || src[0] !== "/") return null;
  for (let i = src.length - 1; i > 0; i--) {
    if (src[i] !== "/") continue;
    let bs = 0; for (let j = i - 1; j >= 0 && src[j] === "\\"; j--) bs++;
...
const balanceBrackets = (src: string): { paren: number; bracket: number; brace: number } => {
  let paren = 0, bracket = 0, brace = 0, inClass = false;
  for (let i = 0; i < src.length; i++) {
    const c = src[i];
    if (c === "\\") { i++; continue; }
    if (c === "[" && !inClass) { inClass = true; bracket++; continue; }
    if (c === "]" && inClass)  { inClass = false; bracket--; continue; }
    if (!inClass) {
...
export function explainRegexError(message: string, pattern: string, flags = "", rawInput?: string): string {
  ...
  if (/(\\p\{|\\u\{)/.test(pattern) && !flags.includes("u")) tips.push("Add 'u' flag for Unicode escapes (\\p{…}, \\u{…}).");
  if (!supportsLookbehind && /(\(\?<=|\(\?<!)/.test(pattern)) tips.push("Lookbehind not supported — refactor to capturing groups.");
  if (/nothing to repeat|invalid quantifier|numbers out of order/i.test(base)) tips.push("Fix quantifiers (*, +, ?, {m,n}) placement/order.");
  if (/range out of order|character class/i.test(base)) tips.push("Inside [...], escape '-' (\\-) or move it to edges; ensure ']' closes the class.");
  if (/invalid group|named capture|group name|reference/i.test(base)) tips.push("Check group syntax/backreferences; avoid (?<name>…) on older runtimes.");
...
export function validateRegexInput(input: string | RegExp, inputFlags?: string | null): RegexValidation {
  ...
  let pattern = literal ? literal.pattern : raw;
  let flags = literal ? literal.flags : (inputFlags ?? "");
