...
function getIPv6AddressType(address: string): 'link-local' | 'unique-local' | 'multicast' | 'global' | 'loopback' | 'unspecified' {
...
function requiresZoneIdentifier(addressType: 'link-local' | 'unique-local' | 'multicast' | 'global' | 'loopback' | 'unspecified'): boolean {
...
function generateSuggestedZones(addressType: 'link-local' | 'unique-local' | 'multicast' | 'global' | 'loopback' | 'unspecified'): string[] {
...
function processIPv6Zone(input: string): IPv6ZoneProcessing {
  try {
    ...
    const withZone = hasZoneId 
      ? `${address}%${zoneId}` 
      : (suggestedZones.length > 0 ? `${address}%${suggestedZones[0]}` : address);
